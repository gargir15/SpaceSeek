  ...
  while true
    ...
    if msgType = "roSGScreenEvent"
      if msg.isScreenClosed() then 
        ' Screen closed, exit loop
        exit while
      else if msg.isScreenVisible() then
        ' Screen is visible, start or resume video playback
        scene.setFocus(true)
        scene.videoControl = "play"
      else
        ' Screen not visible, pause video playback
        scene.videoControl = "pause"
      end if
    else if msgType = "roSGNodeEvent" and msg.GetNode().IsScreenNode()
      ' Video playback ended, restart video
      scene.videoControl = "play"
    end if
  end while
