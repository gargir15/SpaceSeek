function init()
    ...
    m.question = m.top.findNode("question1")
    m.bg = m.top.findNode("level_1_screen_bg")
    m.answers = m.top.findNode("answer1ButtonGroup")
    m.back = m.top.findNode("backButton")
    ...
    m.answers.buttons = ["option 1", "option 2", "option 3"] 
    m.top.observeField("visible", "onVisibleChange")
    m.answers.setFocus(false)
    m.answers.visible = false
    ...
    reset_dialog()
    m.answers.observeField("buttonSelected", "onAnsSelected")
    ...
    m.answers.textFont = m.fonts
    m.answers.focusedTextFont = m.fonts
    m.back.textFont = m.fonts
    m.question.font = m.fonts
    
    m.back.focusedTextFont = m.fonts2
    m.pointsLabel.font = m.fonts2

    '<value>.ToStr(formatStr)

    fetchJson1Data("level1Q1.json")
end function


sub onVisibleChange()
    ? "onVisibleChange function for level 1 called"
...
sub onAnsSelected()
    answerIndex = m.answers.buttonSelected
    
    ? "Selected answer index: "; answerIndex
    ? "checking" ; m.answers.buttonSelected
    ? "Correct answer index: "; m.jsonResponse.correctAns
    
    if m.question.visible = true then 
        if m.answers.buttonSelected = m.jsonResponse.correctAns then
            ...
            ? "correct answer"
            m.points = m.points + 10
            AddAndSetFields(m.global, {Allpoints: m.points})
            m.pointsLabel.text= "points: " + ToString(m.points)

            if m.jsonResponse.num = 3
                m.corrAns = 1
                AddAndSetFields(m.global, {correctAns1: m.corrAns})
                ? "calling Exit Video function"
                playExitVideo()
            else 
                nextQuestion()
            end if
...
sub nextQuestion()
    if m.jsonResponse.num = 3 'if it is on the last question
        ...
        fetchJson1Data(m.jsonResponse.optionpath)
        m.answers.setFocus(true)
    end if
end sub
...
sub onJsonData()
    ...
    if response <> invalid and response <> ""
        ...
        if m.jsonResponse <> invalid
            m.answers.buttons = [m.jsonResponse.option1, m.jsonResponse.option2, m.jsonResponse.option3]
...
function playExitVideo()
    ...
    preloadVideo(m.video, "pkg:/images/first.mov")
    reset_screen()
...
sub onVideoStateChange()
    if m.video.state = "finished"
        ...
        m.bg.uri = "https://kingsfortress-3e1c1a41ac2a.herokuapp.com/images/level_one_bg.jpeg"
    end if
end sub